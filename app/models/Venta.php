...
class Venta
{
    ...
    public function verFilasPeriodoGanancia($periodo)
    {
	  ...
	  $filmoneda = $temoAr[3];
	    ...
	    v.moneda
	     ...
	     if ($filmoneda >0) {
	      $sql = $sql. " AND v.moneda='$filmoneda'";
              }	
             $sql = $sql. " order by v.fecha_emision asc, v.numero asc";
        } elseif ($temoAr[2] == 'nn' ) {



            $periodo = $temoAr[0] . $temoAr[1];
            $sql = "select v.id_venta, v.fecha_emision, ds.abreviatura, v.moneda,
       v.id_tido, v.serie, v.numero, c.documento, c.datos, v.total, v.estado, v.enviado_sunat, vs.nombre_xml,metodo_pago.nombre AS metodo
        from ventas as v
            LEFT JOIN documentos_sunat ds on v.id_tido = ds.id_tido
            LEFT JOIN clientes c on v.id_cliente = c.id_cliente
	...
	$sql = $sql. " order by v.fecha_emision asc, v.numero asc";
	    ...
	    $sql="SELECT  SUM(pv.cantidad*pv.costo) as costo,  
		(CASE WHEN SUM(pv.cantidad*p.costoP) IS NULL THEN SUM(pv.cantidad*pv.costo) ELSE SUM(pv.cantidad*p.costoP) END)  AS costosol
		FROM productos_ventas pv, productos p 
		WHERE pv.id_venta = '{$row['id_venta']}' AND p.id_producto = pv.id_producto";	
            $rest22 = $this->conectar->query($sql);
            $costorTwemp=0;
            $costorTwpen=0;
            if ($temDa=$rest22->fetch_assoc()){
                $costorTwemp=$temDa['costo'];
                $costorTwpen=$temDa['costosol'];
            }
            $row['costo'] = $costorTwemp;
            $row['costosol'] = $costorTwpen;
